...
class DBHelper {
  ...
  void _onCreate(Database db, int version) async {
    ...
    await db.rawInsert(
      "INSERT INTO user (name, username, password) VALUES ('Rosita Ayu Tri Lestari', 'rositaayu', 'password')",
    );
    print('DB Created');
  }
  ...
  Future<String> getPassword() async {
    ...
    List<Map> list = await dbClient!.rawQuery(
      "SELECT password FROM user WHERE username = 'rositaayu'",
    );
  ...
  Future<bool> updatePassword(String password) async {
    ...
    int res = await dbClient!.rawUpdate(
      "UPDATE user SET password = '$password' WHERE username = 'rositaayu'",
    );
